...
export function DOMPanel({ symbol }: { symbol: string; tick?: { bid: number; ask: number } }) {
  ...
  return (
    <section className="flex min-h-0 flex-1 flex-col bg-card" aria-label="Large Order Book">
      <PanelHeader
        title="Large Order Book"
        right={<span className="rounded-full bg-primary/10 px-2 py-1 text-label text-primary">Live</span>}
      >
      ...
      {historyUnavailable && (
        <div className="border-b border-warning/30 bg-warning/10 px-3 py-2 text-meta text-warning">
          Live only · 30-day history unavailable
        </div>
      )}

      <div className="grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_34px] gap-2 border-b border-border px-2.5 py-1.5 text-label uppercase text-muted-foreground">
        ...
        {sorted.length === 0 ? (
          ...
          return (
            <div
              ...
              className={cn("relative grid grid-cols-[minmax(0,1fr)_minmax(0,1fr)_34px] items-center gap-2 overflow-hidden border-b border-border/50 px-2.5 py-1.5 font-mono text-data leading-none [contain-intrinsic-size:auto_18px] [content-visibility:auto] hover:bg-secondary/60", hoveredId === level.id && "bg-secondary ring-1 ring-inset ring-primary/60", !active && "opacity-45")}
            >
